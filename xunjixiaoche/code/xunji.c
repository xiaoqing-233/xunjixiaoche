...
/**
 * @brief 计算黑线的加权位置
 * @return 位置值（0-7的浮点数）；丢线时根据位置历史给出转向方向。
 *
 * 算法说明：
 ...
 * 2. 计算黑线的精确位置（0-7）
 * 3. 设置居中的灰度目标（0.0）
 * 4. PID控制器会对 current_position 相对于传感器中心的偏差进行控制
 *
 * 位置定义：
 * - 传感器0在最左边（位置0.0）
 * - 传感器7在最右边（位置7.0）
 ...
 * - 当所有传感器都检测不到黑线时（直角弯）
 * - 根据历史位置的平均值判断方向（更可靠）
 * - 返回边缘值（0.0或7.0）以保持最近的转向方向
 */
void track_line(void)
{
    track_read_sensors();                      // 1. 读取传感器数据
    weighted_value = calculate_line_position();// 2. 计算黑线位置（0-7）

    // The PID input is centered in pid.c, so zero means the sensor-array center.
